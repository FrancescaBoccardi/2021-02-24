...
public class PremierLeagueDAO {
		...
		try {
			...
			while (res.next()) {
				...
				if(idMap.containsKey(id1) && idMap.containsKey(id2)) {
					result.add(new Adiacenza(idMap.get(id1), idMap.get(id2), res.getDouble("peso")));
				}
			}
	...
	public int getTeamBestPlayer(Player best) {
		
		String sql = "SELECT TeamID FROM actions WHERE PlayerID = ? GROUP BY TeamID";
